...
# then returned to the caller. Note in particular that every time #map yields to the block, 
# it obtains just one value. That value may be a complex value - it may even be another collection 
# - but it is nevertheless just one value. Thus, #map returns an Array that has the same number of 
# elements that the original collection had.

# Write a method called map that behaves similarly for Arrays. It should take an 
# Array as an argument, and a block. It should return a new Array that contains the 
# return values produced by the block for each element of the original Array.
...
# - yield each element from array to block
# - push block return value to result 
# - repeat until iterated through all objects 
# - return result array 
